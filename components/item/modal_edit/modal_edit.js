...
const ModalEdit = ({ editItem, reference_item }) => {
  const ITEM_INITIAL_STATE = {
    reference_id: reference_item.id,
    item_name: reference_item.name,
    item_description: reference_item.description,
    item_price: reference_item.price,
    item_order: reference_item.order,
    media_id:
      reference_item.media.length > 0 ? reference_item.media[0].media_id : 0,
    media: {},
    type: "image",
    name: "",
    description: "",
  ...
  const handleSubmit = () => {
    ...
    data_send.append("item_order", item.item_order);
    data_send.append("media_id", item.media_id);
    ...
    editItem(data_send).then((result) => {
      handleClose();
    });
  };

  const openFileManager = () => {
    let input = document.getElementById(`regular_item_edit`);
    input.click();
  };
  ...
  return (
    <>
          ...
          <Row md={12} sm={12} xs={12}>
            ...
            >
              ...
              <div style={{ height: 0, width: 0, overflow: "hidden" }}>
                <input
                  id="regular_item_edit"
                  type="file"
                  name="regular_item_edit"
                  accept="image/*"
                  onChange={fileAccept}
                />
              </div>
                ...
                >
                  <Row md={12} sm={12} xs={12}>
                    <Col md={12} sm={12} xs={12} className={`left`}>
                      <img
                        src={reference_item.media[0].url_regular}
                        alt="img"
                        className={styles.file_img}
                      />
                      <a
                        target="_blank"
                        href={reference_item.media[0].url_regular}
      ...
      <p className={styles.open_modal} onClick={handleOpen} title="Editar Ítem">
